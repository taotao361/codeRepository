...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {
    ...
    1、应用启动过程中，会依次调用delegate已经启动完成和已经获得焦点的方法，不会调用已经进入前台的方法；
    2、单击home键进入后台会依次调用delegate的将要失去焦点的方法和已经进入后台的方法；
    3、从后台进入前台(无论是双击home键进入或者点击应用图标进入)，会回调delegate的将要进入前台方法；紧接着会继续回调delegate的已经获得焦点的方法，证明程序已经获得了焦点；
    4、在前台，双击home键，只会调用delegate的将要失去焦点的方法，证明程序将要失去焦点。当用户真正切换应用时候，才会继续调用delegate的已经进入后台的方法，证明程序已经进入后台。
    5、当双击HOME键，上划杀死程序时，先调用 
        1.取消焦点（ applicationWillResignActive） 
        2、进入后台（ applicationDidEnterBackground） 
        3、进程退出方法（ applicationWillTerminate）；
    6、下拉通知栏时，只会调用失去焦点方法（），程序并没有进入后台；
    7、同样，从屏幕下方向上滑动屏幕时，同样只调用失去焦点方法；收起工具栏时调用 只 获得焦点方法；
